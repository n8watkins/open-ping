...
Changing the key in place makes values encrypted with the previous key unreadable.
...
- Discord webhook URLs and generic webhook HMAC secrets are encrypted inside notification-channel configuration.
...
The generic webhook destination URL, email channel addresses, and other non-secret notification-channel fields are not encrypted unless listed above.
...
- Notification outbox targets, event types, payloads, retry state, and errors.
...
Notification payloads and operational errors can contain monitor names, URLs, incident details, email addresses, or other deployment-specific information.
...
- An older notification-channel secret is encrypted the next time that channel is updated with `MASTER_KEY` configured.
...
For immediate hardening after an upgrade, re-save monitors and notification channels, re-register push devices, and rotate heartbeat tokens.
